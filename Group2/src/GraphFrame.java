...
class GraphFrame extends MinyFrame {
	...
	GraphFrame(appGUI appGUI, MinyGUI parent, String name, InterpolatedFloat value,
			...
			if (_selected != -1) {
				if (this.appGUI.pmouseX != this.appGUI.mouseX) {
					if (this.appGUI.mouseX < r.x && !_value.limitXMin()) {
						float t = appGUI.max(
								0,
								appGUI.map(this.appGUI.pmouseX - this.appGUI.mouseX, 0, r.w, 0, _maxX
										- _minX));
						if (t > 0) {
						...
						float t = appGUI.max(
								0,
								appGUI.map(this.appGUI.mouseX - this.appGUI.pmouseX, 0, r.w, 0, _maxX
										- _minX));
						if (t > 0) {
							float v = _value.getPosition(_selected) + t;
							_value.setPosition(_selected, v);
						...
						float p = appGUI.constrain(
								appGUI.map(this.appGUI.mouseX, r.x, r.x + r.w, _minX, _maxX),
								_minX, _maxX);
				...
				if (this.appGUI.pmouseY != this.appGUI.mouseY) {
					if (this.appGUI.mouseY < r.y && !_value.limitYMax()) {
						float t = appGUI.max(
								0,
								appGUI.map(this.appGUI.pmouseY - this.appGUI.mouseY, 0, r.h, 0, _maxY
										- _minY));
						if (t > 0) {
						...
						float t = appGUI.max(
								0,
								appGUI.map(this.appGUI.mouseY - this.appGUI.pmouseY, 0, r.h, 0, _maxY
										- _minY));
						if (t > 0) {
							float v = _value.getValue(_selected) - t;
							_value.setValue(_selected, v);
						...
						float v = appGUI.constrain(
								appGUI.map(this.appGUI.mouseY, r.y + r.h, r.y, _minY, _maxY),
								_minY, _maxY);
						_value.setValue(_selected, v);
					}
					updateGraph();
	...
	public void onMousePressed() {
		...
		if (_parent.getModCtrl()) {
			Rect r = getGraphArea();
			float p = appGUI.constrain(appGUI.map(this.appGUI.mouseX, r.x, r.x + r.w, _minX, _maxX),
					_minX, _maxX);
			float v = appGUI.constrain(appGUI.map(this.appGUI.mouseY, r.y + r.h, r.y, _minY, _maxY),
					_minY, _maxY);
