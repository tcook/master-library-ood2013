...
public class GraphFrame extends MinyFrame
{
	...
	GraphFrame(appGUI appGUI, MinyGUI parent, String name, InterpolatedFloat value, FrameCreator property)
	{
	...
	{
		Rect r = getGraphArea();
		this.appGUI.stroke(_parent.fg); this.appGUI.fill(_parent.fg);
		...
		for(int i=0; i<nb; i++)
		{
			...
			if(_drawGrid) { this.appGUI.stroke(gridColor); this.appGUI.line(sx, r.y, sx, r.y+r.h); }
		...
		for(int i=0; i<nb; i++)
		{
			...
			if(_drawGrid) { this.appGUI.stroke(gridColor); this.appGUI.line(r.x, sy, r.x+r.w, sy); }
			this.appGUI.stroke(_parent.fg);
			...
			else
			{
				...
				this.appGUI.text(caption, area.x, sy-10, r.x-area.x-10, 20);
			}
		}
	...
	public void display()
	{
		...
		this.appGUI.noFill(); this.appGUI.stroke(_parent.fg);
		Rect r = getGraphArea();
		this.appGUI.rect(r);
		...
		{
			float x = r.x + appGUI.map(_value.getPosition(i), _minX, _maxX, 0, r.w);
			float y = r.y + appGUI.map(_value.getValue(i), _minY, _maxY, r.h, 0);
			if(_selected == i) this.appGUI.fill(_parent.selectColor); else this.appGUI.fill(_parent.bg);
		...
		if(this.appGUI.mousePressed)
		{
			if(_selected != -1)
			{
				if(this.appGUI.pmouseX != this.appGUI.mouseX)
				{
					...
					{
						float t = appGUI.max(0,appGUI.map(this.appGUI.pmouseX-this.appGUI.mouseX, 0, r.w, 0, _maxX-_minX));
						if(t > 0)
						{
					...
					else if(this.appGUI.mouseX > r.x+r.w && !_value.limitXMax())
					{
						float t = appGUI.max(0,appGUI.map(this.appGUI.mouseX-this.appGUI.pmouseX, 0, r.w, 0, _maxX-_minX));
						if(t > 0)
						{
							float v = _value.getPosition(_selected) + t;
							_value.setPosition(_selected, v);
					...
					{
						float p = appGUI.constrain( appGUI.map(this.appGUI.mouseX, r.x, r.x+r.w, _minX, _maxX), _minX, _maxX);
				...
				if(this.appGUI.pmouseY != this.appGUI.mouseY)
				{
					...
					{
						float t = appGUI.max(0,appGUI.map(this.appGUI.pmouseY-this.appGUI.mouseY, 0, r.h, 0, _maxY-_minY));
						if(t > 0)
						{
					...
					else if(this.appGUI.mouseY > r.y+r.h && !_value.limitYMin())
					{
						float t = appGUI.max(0,appGUI.map(this.appGUI.mouseY-this.appGUI.pmouseY, 0, r.h, 0, _maxY-_minY));
						if(t > 0)
						{
							float v = _value.getValue(_selected) - t;
							_value.setValue(_selected, v);
					...
					{
						float v = appGUI.constrain( appGUI.map(this.appGUI.mouseY, r.y+r.h, r.y, _minY, _maxY), _minY, _maxY);
						_value.setValue(_selected, v);
					}
					updateGraph();
				}
