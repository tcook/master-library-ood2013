...
public class appParticleEditor extends JFrame {
	...
	public class Effects extends Canvas implements KeyListener {
		  ...
		  public Effects() {addKeyListener(this);}
	...
	public void reloadRows () {
		/*EventQueue.invokeLater(new Runnable() {
			...
			public void run () {*/
				...
				addRow(new vgu_ScaledNumericPanel("Emission", "Duration", emitter.getEmission()));
				addRow(new vgu_ScaledNumericPanel("Life", "Duration", emitter.getLife()));
				addRow(new vgu_ScaledNumericPanel("Life Offset", "Duration", emitter.getLifeOffset()));
				...
				addRow(new vgu_SpawnPanel(emitter.getSpawnShape(), appParticleEditor.this));
				addRow(new vgu_ScaledNumericPanel("Spawn Width", "Duration", emitter.getSpawnWidth()));
				addRow(new vgu_ScaledNumericPanel("Spawn Height", "Duration", emitter.getSpawnHeight()));
				addRow(new vgu_ScaledNumericPanel("Size", "Life", emitter.getScale()));
				addRow(new vgu_ScaledNumericPanel("Velocity", "Life", emitter.getVelocity()));
				addRow(new vgu_ScaledNumericPanel("Angle", "Life", emitter.getAngle()));
				addRow(new vgu_ScaledNumericPanel("Rotation", "Life", emitter.getRotation()));
				addRow(new vgu_ScaledNumericPanel("Wind", "Life", emitter.getWind()));
				addRow(new vgu_ScaledNumericPanel("Gravity", "Life", emitter.getGravity()));
				addRow(new vgu_GradientPanel("Tint", emitter.getTint()));
				addRow(new vgu_PercentagePanel("Transparency", "Life", emitter.getTransparency()));
				addRow(new vgu_OptionsPanel(appParticleEditor.this));
				for (Component component : rowsPanel.getComponents())
					if (component instanceof vgu_EditorPanel) ((vgu_EditorPanel)component).update(appParticleEditor.this);
				rowsPanel.repaint();
				rowsPanel.validate();
			/*}
		});*/
	...
	public void setVisible (String name, boolean visible) {
		for (Component component : rowsPanel.getComponents())
			if (component instanceof vgu_EditorPanel && ((vgu_EditorPanel)component).getName().equals(name)) component.setVisible(visible);
	}
	...
	private void initializeComponents () {
		...
		}
		{
			...
			splitPane.add(leftSplit, JSplitPane.LEFT);
			{
				...
				if (lwjglCanvas!=null) spacer.add(lwjglCanvas);
