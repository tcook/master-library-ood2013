...
public class appGUI extends PApplet {
	 ...
	 * (it's click in color area to add a new point to gradient, drag point to far left to delete, point 0 cannot be moved/deleted)
	...
	public void mousePressed() { gui.onMousePressed();  }
	public void keyPressed()   { gui.onKeyPressed();    }
	public void keyReleased()  { gui.onKeyReleased();   }
	PImage img;
	/*public void setup()
	{
		size(600, 400);
	...
	public class TestButton implements ButtonCallback
	{ public void onButtonPressed() { time = 0; running.setValue(true); } }
	...
	public class GradientFrame extends MinyFrame implements FrameCreator
	{
			...
			if(_parent.hasLock(this))
			{
				...
				else if(_editingPos != -1 && pmouseX != mouseX)
				{
					...
					if (p==0 && _editingPos!=0) {
						if(_frame != null) _parent.removeFrame(_frame);
						_value.remove(_editingPos);
						_selected = -1;
						_editingPos = -1;
					} else if (_editingPos!=0) _value.setPosition(_editingPos, p);
					updateGradient();
				}
			}
		}
		...
		/*public void onKeyPressed()
	  {
	    ...
	    if(_selected != -1 && (keyCode == DELETE || keyCode == BACKSPACE)) //key == CODED &&
	    {
		...
		public void onCloseFrame(MinyFrame frame) { _editingColor=-1; _frame=null; }
	}
		...
		private void updateLimits()
		{
			...
			if(size <= 1)
			{
				...
				else
				{
					...
					tXMin = p - 0.5f; tXMax = p + 0.5f;
				}

				float v = getValue(0);
				tYMin = v - 0.5f; tYMax = v + 0.5f;
			}
			else
			{
	...
	public class PropertyGraph extends Property implements FrameCreator
	{ 
		private InterpolatedFloat _value;
		...
		void updateGraph()
		{
			...
			_graph.fill(-1); _graph.rect(0,0,r.w,r.h);
		...
		public void onCloseFrame(MinyFrame frame) { _frame = null; updateGraph(); }
	}
	//---------------------------------------------------
	...
	public class Property implements MinyWidget
	{
		...
		public MinyValue get() {return new MinyString("");}
		public void update() {}
		public void display(PGraphics pg, int y)
		{
		...
		public void getFocus() { _hasFocus=true; }
		public void lostFocus() { _hasFocus=false; }
		public void onMousePressed() {}
		public void onKeyPressed() {}
	}

	public class MinyFrame implements MinyWidget
	{
		protected Rect _box;
		protected MinyGUI _parent;
		...
		public Rect getRect() { return _box; }
	...
	public class PropertyButton extends Property
	{
		...
		public MinyValue get() {return new MinyBoolean(_pressed);}
		Rect getBox()
	...
	public class VScrollbar implements MinyWidget
	{
		...
		public Rect getRect() { return _area; }
		void setPosition(Rect r) { _area = r; }
		Rect getBox()
	...
	public class PropertyColorChooser extends Property implements FrameCreator
	{ 
		...
		public void onCloseFrame(MinyFrame frame) { _frame = null; }
	}
	...
	public class ColorChooserFrame extends MinyFrame
	{
		...
		void onClose() { _property.onCloseFrame(this); }
		public void onKeyPressed() { _parent.removeFrame(this); }
	...
	public class MinyInteger implements MinyValue
	{
		private Integer _v;
		MinyInteger() {_v=new Integer(0); }
	...
	public class MinyFloat implements MinyValue
	{
		private Float _v;
		MinyFloat() {_v=new Float(0); }
	...
	public class MinyBoolean implements MinyValue
	{
		private Boolean _v;
		MinyBoolean() {_v=new Boolean(false); }
	...
	public class MinyString implements MinyValue
	{
		private String _v;
		MinyString() {_v=new String(""); }
	...
	public class MinyColor implements MinyValue
	{
		private int _v;
		MinyColor() {_v= -1;}
		MinyColor(int v) { _v = v; }
		int getValue() { return _v; }
		void setValue(int v) { _v = v; }
		public String getString() { return "0x"+hex(_v); }
