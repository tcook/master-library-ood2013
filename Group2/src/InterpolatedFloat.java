...
public class InterpolatedFloat
{
	...
	{
		if(_limitXMin) pos = appGUI.max(_XMin, pos);
		if(_limitXMax) pos = appGUI.min(_XMax, pos);
		if(_limitYMin) val = appGUI.max(_YMin, val);
		if(_limitYMax) val = appGUI.min(_YMax, val);
	...
	{  
		...
		if(_limitXMax) pos = appGUI.min(_XMax, pos);
		if(_limitYMin) val = appGUI.max(_YMin, val);
		if(_limitYMax) val = appGUI.min(_YMin, val);
		var.position = pos;
		var.value = val;
		update();
	}
	...
	float get(float pos)
	{
		...
		if(_useLookupTable)
		{
			float p = appGUI.map(pos, _lookupMin, _lookupMax, 0, 1)*_lookupTable.length;
			int i = appGUI.constrain(appGUI.floor(p), 0, _lookupTable.length-1);
		...
		case 0:
		default:
			return appGUI.lerp(leftValue, rightValue, (1-appGUI.cos(amt*appGUI.PI))/2);
		case 1:
	...
	private void update()
	{
		...
		{
			float p = appGUI.map(i, 0, _lookupTable.length-1, _lookupMin, _lookupMax);
