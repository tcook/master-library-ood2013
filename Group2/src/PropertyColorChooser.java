...
public class PropertyColorChooser extends Property implements appGUI.FrameCreator
{ 
	/**
	 * 
	 */
	...
	public PropertyColorChooser(appGUI appGUI, String name, MinyColor value)
	{
		...
		for(int x=0; x<h; x++)
		{
			for(int y=0; y<h; y++)
			{
				boolean b = (x%h < h/2);
				if(y%h < h/2) b = !b;
				_imgChecker.pixels[y*h+x]=b?this.appGUI.color(160):this.appGUI.color(95);
			}
		}
	...
	public void onMousePressed()
	{
		if(this.appGUI.overRect(getBox()))
		{
			if(_frame == null)
			{
				_frame = new ColorChooserFrame(this.appGUI, _parent, _name, _value, this);
