...
public class MinyFrame implements appGUI.MinyWidget
{
	...
	public void update()
	{
		...
		if(this.appGUI.mousePressed)
		{
			...
			if(_resizing != -1 && (this.appGUI.pmouseX != this.appGUI.mouseX || this.appGUI.pmouseY != this.appGUI.mouseY))
			{
				if((_resizing & 1) != 0)
				{
					_box.x = appGUI.min(_saved.x+_saved.w-_minW, _saved.x + this.appGUI.mouseX - _clickX);
					_box.w = appGUI.max(_minW, _saved.w - this.appGUI.mouseX + _clickX);
				}
				...
				if((_resizing & 4) != 0)
				{
					_box.y = appGUI.min(_saved.y+_saved.h-_minH, _saved.y + this.appGUI.mouseY - _clickY);
					_box.h = appGUI.max(_minH, _saved.h - this.appGUI.mouseY + _clickY);
				}
	...
	public void display()
	{
		this.appGUI.fill(_parent.bg); this.appGUI.stroke(_parent.fg);
		Rect r = new Rect(this.appGUI, _box);
		if(_resizeable && _moveable) 
		{
			...
			this.appGUI.rect(r); r.grow(-1);
			this.appGUI.rect(r); r.grow(-1);
			r.y+=18; r.h-=18; 
			this.appGUI.stroke(_parent.fg); this.appGUI.fill(_parent.bg);
			this.appGUI.rect(r);
		} 
		else if(_resizeable) 
		{
		...
		if(_moveable)
		{
			...
			if(_hasFocus) this.appGUI.fill(_parent.selectColor); else this.appGUI.fill(_parent.bg);
			this.appGUI.rect(r);
			...
			this.appGUI.fill(_parent.fg); this.appGUI.noStroke();
			r.x += 5;
			r.w -= 10;
			...
			r.w = r.h;
			this.appGUI.noFill();
			if(_hasFocus) this.appGUI.stroke(_parent.selectColor); else this.appGUI.stroke(_parent.bg);
			r.grow(-1);
			this.appGUI.rect(r);
			r.grow(-1);
			this.appGUI.rect(r);
