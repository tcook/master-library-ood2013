...
public class dg_ParticleBasic extends dg_Particle {
    ...
    public static final int lifeTime=dg_NameSpaceParticles.nsp.add("lifeTime");
    public static final int lifeTimeMax=dg_NameSpaceParticles.nsp.add("lifeTimeMax");
    public static final int frictionA=dg_NameSpaceParticles.nsp.add("frictionA");
    public static final int frictionV=dg_NameSpaceParticles.nsp.add("frictionV");
	...
	public void draw(PGraphics g) {
		if (p[dead]!=0) return;
		g.stroke(color1);
		//TODO weight>diameter increases diameter but leaves weight at maximum of old diameter
	    g.strokeWeight((float) (2*p[w]*(1+2*p[lifeTime]*Math.log(p[lifeTime])/p[lifeTimeMax])));
	    g.ellipse(p[x], p[y], Math.abs(p[prevx]-p[x])+1, Math.abs(p[prevy]-p[y])+1);
	}
	public void move() {	    
	    //Aging:
	    ...
	    //Movement:
	    p[ax]*=p[frictionA];  p[ay]*=p[frictionA];
	    p[vx]*=p[frictionV];  p[vy]*=p[frictionV];
	    p[vx]+=p[ax];  p[vy]+=p[ay];
	    p[x]+=p[vx];  p[y]+=p[vy];
	  }
}
