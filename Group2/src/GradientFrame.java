...
public class GradientFrame extends MinyFrame implements appGUI.FrameCreator
{
	...
	GradientFrame(appGUI appGUI, MinyGUI parent, String name, ColorGradient value, PropertyGradient property)
	{
		...
		if (appGUI._imgChecker==null) {
			...
			for(int x=0; x<appGUI._imgChecker.width; x++)
			{
				for(int y=0; y<appGUI._imgChecker.height; y++)
				{
					boolean b = (x%20 < 10);
					if(y%20 < 10) b = !b;
					appGUI._imgChecker.pixels[y*appGUI._imgChecker.width+x]=b?this.appGUI.color(160):this.appGUI.color(95);
				}
			}
			appGUI._imgChecker.updatePixels();
		}
		if (appGUI._imgSmallChecker==null) {
			appGUI._imgSmallChecker = this.appGUI.createImage(15, 15, appGUI.RGB);
			...
			for(int x=0; x<appGUI._imgSmallChecker.width; x++)
			{
				for(int y=0; y<appGUI._imgSmallChecker.height; y++)
				{
					boolean b = (x < 8);
					if(y < 8) b = !b;
					appGUI._imgSmallChecker.pixels[y*appGUI._imgSmallChecker.width+x]=b?this.appGUI.color(160):this.appGUI.color(95);
				}
			}
	...
	public void update()
	{
		...
		if(_parent.hasLock(this))
		{
			...
			else if(_editingPos != -1 && this.appGUI.pmouseX != this.appGUI.mouseX)
			{
				Rect r = getClientArea();
				//int w = getBox().w;
				float p = appGUI.constrain(appGUI.map(this.appGUI.mouseX-r.x, 0, r.w, 0, 1), 0, 1);
	...
	public void onMousePressed()
	{
		...
		{
			int x = r.x+appGUI.floor(appGUI.map(_value.getPosition(i), 0, 1, 0, r.w-1));
			int y = r.y+r.h;
			if(this.appGUI.overRect(x-8, y+24, 16, 16))
			{
				...
				if(_frame == null)
				{
					_frame = new ColorChooserFrame(this.appGUI, _parent, _name + " (" + i + ")", _color, this);
	...
	/*public void onKeyPressed()
  {
	...
	public void display()
	{
		...
		this.appGUI.stroke(_parent.fg); this.appGUI.noFill();
		r.x--; r.y--; r.w++; r.h++;
		this.appGUI.rect(r);
		...
		for(int i=0; i<_value.size(); i++)
		{
			int x = r.x+10+appGUI.floor(appGUI.map(_value.getPosition(i), 0, 1, 0, w-1));
			int y = r.y+35;

			if(_selected == i) this.appGUI.fill(_parent.selectColor);
			else this.appGUI.noFill();
