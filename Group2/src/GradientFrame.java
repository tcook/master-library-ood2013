...
class GradientFrame extends MinyFrame implements appGUI.FrameCreator {
	/**
	 * 
	 */
	...
	GradientFrame(appGUI appGUI, MinyGUI parent, String name, ColorGradient value,
		...
		if (appGUI._imgChecker == null) {
			...
			for (int x = 0; x < appGUI._imgChecker.width; x++) {
				for (int y = 0; y < appGUI._imgChecker.height; y++) {
					...
					if (y % 20 < 10)
						b = !b;
					appGUI._imgChecker.pixels[y * appGUI._imgChecker.width + x] = b ? this.appGUI.color(160)
		...
		if (appGUI._imgSmallChecker == null) {
			...
			for (int x = 0; x < appGUI._imgSmallChecker.width; x++) {
				for (int y = 0; y < appGUI._imgSmallChecker.height; y++) {
					...
					if (y < 8)
						b = !b;
					appGUI._imgSmallChecker.pixels[y * appGUI._imgSmallChecker.width + x] = b ? this.appGUI.color(160)
							: this.appGUI.color(95);
				}
			}
	...
	public void onMousePressed() {
		...
		for (int i = 0; i < _value.size(); i++) {
			int x = r.x
					+ appGUI.floor(appGUI.map(_value.getPosition(i), 0, 1, 0, r.w - 1));
			int y = r.y + r.h;
	...
	public void display() {
		...
		for (int i = 0; i < _value.size(); i++) {
			int x = r.x + 10
					+ appGUI.floor(appGUI.map(_value.getPosition(i), 0, 1, 0, w - 1));
			int y = r.y + 35;
