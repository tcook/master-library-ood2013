...
public class ColorChooserFrame extends MinyFrame
{
	...
	ColorChooserFrame(appGUI appGUI, MinyGUI parent, String name, MinyColor value, FrameCreator property)
	{
		...
		for(int x=0; x<256; x++)
		{
			for(int y=0; y<20; y++)
			{
				boolean b = (x%20 < 10);
				if(y%20 < 10) b = !b;
				_imgChecker.pixels[y*256+x]=b?this.appGUI.color(160):this.appGUI.color(95);
			}
		}
	...
	public void display()
	{
		...
		this.appGUI.image(_imgR, x, y); y+= 25;
		this.appGUI.image(_imgG, x, y); y+= 25;
		this.appGUI.image(_imgB, x, y); y+= 25;
		...
		_pg.noStroke();
		_pg.rect(0, 0, 256, 97);
		_pg.fill(this.appGUI.color(0)); _pg.stroke(this.appGUI.color(255)); 
		...
		this.appGUI.rect(area); area.y += 25;
		this.appGUI.rect(area); area.y += 25;
		this.appGUI.rect(area); area.y += 25;
		...
		this.appGUI.noStroke(); this.appGUI.fill(_parent.fg);
		this.appGUI.text("R", area); area.y+=25;
		this.appGUI.text("G", area); area.y+=25;
		this.appGUI.text("B", area); area.y+=25;
