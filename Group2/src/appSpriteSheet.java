...
public class appSpriteSheet {
	public static void main(String args[]) { //combines all images in directory into a single sprite sheet
		...
		for (String file : dir.list()) {
			...
			ww=Math.min(ww,img.getWidth()); hh=Math.min(hh,img.getHeight());
			img.flush();
			n++;
		} //for
		System.out.println(n+" "+ww+" "+hh);
		BufferedImage img2=new BufferedImage(n*ww,hh,BufferedImage.TYPE_INT_ARGB);
		int nn=0;
		for (String file : dir.list()) {
			File f=null;
			if (file.charAt(0)=='.') continue;
			try {
				f=new File(directory+file);
				img = ImageIO.read(f);
				if (ww!=img.getWidth()) return; 
				if (hh!=img.getHeight()) return;
