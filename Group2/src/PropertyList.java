...
public class PropertyList extends Property
{
	...
	Rect getBox()
	{ return new Rect(this.appGUI, (int)(_x+_w*0.4+3), _y+2, (int)(_w*0.6-8), 18); }
	...
	public void onMousePressed()
	{
		if(this.appGUI.overRect(getBox()))
		{
			...
			Rect r = new Rect(this.appGUI, b.x, b.y+(below?b.h:-(b.h-2)*_choices.length-2), b.w, 2+(b.h-2)*(_choices.length));
			PropertyListFrame frame = new PropertyListFrame(this.appGUI, _parent, _name, _value, _choices);
			frame.setRect(r);
	...
	public ff_XMLWriter writeXML(ff_XMLWriter xml, int i){
		...
		try{
			...
			.attribute("type","PropertyList")
			.element("name", _name)
			.element("_value", _value._v)
			.element("rawchoices", rawchoices)
			.pop();
		} 	catch (IOException e) {
