...
class BlockCity implements HieghtField {
	 ...
	 * 
	 */
	private terrainmarch BlockCity;
	...
	public float GetHieght(float x, float z) {
		...
		float h = BlockCity.noise(terrainmarch.floor(x + .5f),
				terrainmarch.floor(z + .5f));
		return terrainmarch.min(terrainmarch.abs(xf - .5f) * 3, 1.f)
				* terrainmarch.min(terrainmarch.abs(xz - .5f) * 3, 1) * 3.f * h;
	}
