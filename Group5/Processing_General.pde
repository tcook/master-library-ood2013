...
* HEX_WIDTH;
private final static int MAP_HEIGHT = MAP_ROWS * HEX_HEIGHT + HEX_HEIGHT;
...
private final static int BITMAP_DATA_SIZE = BITMAP_DATA_WIDTH * BITMAP_DATA_HEIGHT;
...
private final static String BITMAP_DATA = "xx....xx" + "x......x"
+ "........" + "........" + "........" + "........" + "x......x"
+ "xx....xx" + "........" + "........" + "...xx..." + ".xxxx..."
+ "...xx..." + "...xx..." + "........" + "........" + "........"
+ "........" + "........" + "....xxx." + "xxxxxxxx" + "...xxxxx"
+ "...xxxxx" + "........" + "x......." + "xx......" + ".xx....."
+ "..xx...." + "...xxx.." + "..xxxx.." + "..x.xxx." + "...xx.xx"
+ "........" + "......xx" + "....xxxx" + "......xx" + "xxxxx.xx"
+ "x.x.x..." + "x.x.x..." + "........" + "........" + "........"
+ "..xxxx.." + "..xx.x.." + "..x.xx.." + "..xxxx.." + "........"
+ "........" + ".x....x." + "x......x" + "........" + "........"
...
private final static int CAMPAIGN_FRIENDLY_COLUMNS = FIRST_POSSIBLE_RIVER_COLUMN;
...
private final static byte[] campaign = new byte[] { 
  0x29, 0x33, 0x39, 0x30, 
  0x1f, 0x22, 0x2b, 0x22, 0x22, 0x34, 0x7a, 0x74, 0x35, 0x42, 0x71, 
  0x45, 0x52, 0x01, 0x55, 0x62, 0x11, 0x65, 0xffffff90, 0x32, 0x0a, 
  0x15, 0x31, 0x19, 0x74, 0x4a, 0x53, 0x54, 0xffffff9f, 0x55, 0x57, 
  0xffffffa4,
};
private final static int LAST_CAMPAIGN_SCENARIO = 11;
private final static int COLOR_FRIENDLY_BACKGROUND = #386bb9;
private final static int COLOR_FRIENDLY_BORDER = #0d38b9;
private final static int COLOR_ENEMY_BACKGROUND = #e5a047;
private final static int COLOR_ENEMY_BORDER = #b93838;
private final static int COLOR_MOVED_BORDER = #878787;
private final static int COLOR_OPEN = #ffffed;
private final static int COLOR_WOODS_1 = #3a691d;
private final static int COLOR_WOODS_2 = #62875b;
private final static int COLOR_WATER_1 = #62a7a7;
private final static int COLOR_WATER_2 = #404aa7;
private final static int COLOR_CITY_1 = #624848;
private final static int COLOR_CITY_2 = #a78787;
private final static int COLOR_RED_1 = #ff0037;
private final static int COLOR_RED_2 = #ff4037;
private final static int COLOR_GREEN_1 = #62ff00;
private final static int COLOR_GREEN_2 = #3aa700;
...
private final static int TERRAIN_DATA_SIZE = TERRAIN_MOVE_COST_OFFSET + 1;
private final static int[] terrainData = new int[] { 
  0, 0, 0, 0, 0, 0, 9, 
  1, COLOR_WOODS_1, COLOR_WOODS_2, HEX_WIDTH * 7 / 10, 4, 4, 0x87, 6, 
  2, COLOR_CITY_1, COLOR_CITY_2, HEX_WIDTH * 4 / 5, 7, 7, 45, 5, 1, 
  COLOR_WATER_1, COLOR_WATER_2, HEX_WIDTH / 4, 3, HEX_HEIGHT / 3, 30, 
  9, 13
};
private final static int SCREEN_MAP_Y = BITMAP_HEIGHT;
private final static int SCREEN_WIDTH = MAP_WIDTH;
private final static int SCREEN_HEIGHT = MAP_HEIGHT + SCREEN_MAP_Y + 2;
...
private final static byte SELECT_ENEMY_UNIT_TO_MOVE = 15 | SEARCH_STATE_BIT;
private final static byte FIND_ENEMY_ATTACK_TARGETS = 16 | SEARCH_STATE_BIT;
private final static byte FIND_FRIENDLY_THREAT = 17 | SEARCH_STATE_BIT;
private final static byte FIND_ENEMY_THREAT = 18 | SEARCH_STATE_BIT;
private final static byte FIND_ENEMY_VALUE = 19 | SEARCH_STATE_BIT;
...
private final static int MAP_DATA_TOTAL_SIZE = MAP_COLUMNS * MAP_ROWS
* MAP_DATA_SIZE;
private final byte[] map = new byte[MAP_DATA_TOTAL_SIZE];
private final static byte FOV_RANGE = 6;
...
private final static int UNIT_SIZE = UNIT_MOVE_OFFSET + 1;
...
public void setup() {
  g = createGraphics(SCREEN_WIDTH, SCREEN_HEIGHT);
  size(780, 506);
  state = NEW_LEVEL;
  turn = 1;
  ...
  for (int n = 0; n < nrRivers; n++) {
    ...
    for (int r = 0; r < MAP_ROWS; r++) {
      map[(c * MAP_ROWS + r) * MAP_DATA_SIZE + MAP_TERRAIN_LAYER] = WATER;
    }
  }
  int firstEnemyColumn = CAMPAIGN_FRIENDLY_COLUMNS + CAMPAIGN_MIN_NML
    + funitsRnd.nextInt(MAP_COLUMNS / 4);
  i = 0;
  for (int c = 0; c < MAP_COLUMNS - 1; c++) {
    for (int r = 0; r < MAP_ROWS; r++) {
      byte terrain = map[i + MAP_TERRAIN_LAYER];
      if (terrain != WATER && r > 0 && c > 0) {
        int woods_percentage = WOODS_BASE;
        if (map[i - MAP_DATA_SIZE * MAP_ROWS + MAP_TERRAIN_LAYER] == WOODS) {
      ...
      byte unit = NO_UNIT;
      if ((c < CAMPAIGN_FRIENDLY_COLUMNS || level == 3
        && c > (MAP_COLUMNS - CAMPAIGN_FRIENDLY_COLUMNS))
        && r > 0) {
        ...
        if (terrain == CITY || u < CAMPAIGN_ENEMY_INFANTRY) {
          if (map[i + MAP_ROWS * MAP_DATA_SIZE
            ...
            if (eunitsRnd.nextInt(2) == 0 && terrain == CITY) {
...
void draw(){
  ...
  g.fill(#ffffff);
  g.text("TURN      / 15", SCREEN_TURN_LABEL_X, 
  SCREEN_STATUS_BAR_TEXT_Y);
  g.text(Integer.toString(turn), SCREEN_TURN_NR_X, 
  SCREEN_STATUS_BAR_TEXT_Y);
  g.text("MAP", SCREEN_LEVEL_LABEL_X, SCREEN_STATUS_BAR_TEXT_Y);
  g.text(Integer.toString(level + 1), SCREEN_LEVEL_NR_X, 
  SCREEN_STATUS_BAR_TEXT_Y);
  if (state == FAIL_LEVEL) {
    g.text("FAIL", SCREEN_MESSAGE_X, SCREEN_STATUS_BAR_TEXT_Y);
  }
  if (state == WON_LEVEL) {
    g.text("WON", SCREEN_MESSAGE_X, SCREEN_STATUS_BAR_TEXT_Y);
  }
  if (state == WON_CAMPAIGN) {
    g.text("WON ALL", SCREEN_MESSAGE_X, SCREEN_STATUS_BAR_TEXT_Y);
  }
  g.fill(COLOR_FRIENDLY_BORDER);
  g.rect(SCREEN_BUTTON_X, 0, SCREEN_WIDTH, SCREEN_BUTTON_HEIGHT);
  g.fill(COLOR_FRIENDLY_BACKGROUND);
  g.rect(SCREEN_BUTTON_X + 2, 2, SCREEN_WIDTH - 4, 
  SCREEN_BUTTON_HEIGHT - 4);
  g.fill(#ffffff);
  g.text(">>", SCREEN_BUTTON_TEXT_X, SCREEN_STATUS_BAR_TEXT_Y - 2);
  int i = 0;
  int x = MAP_FIRST_COLUMN_CENTER_X;
  for (int c = 0; c < MAP_COLUMNS; c++) {
    int y = ((c % 2 == 0) ? MAP_EVEN_ROW_LAYER : 0) + SCREEN_MAP_Y + HEX_HEIGHT / 2;
    for (int r = 0; r < MAP_ROWS; r++) {
      g.fill(COLOR_OPEN);
      g.noStroke();
      for (int hx = 0; hx <= HEX_WIDTH / 4; hx++) {
        int hy = hx * 155 / 100;
        g.rect(x - HEX_WIDTH / 2 + hx, y - hy + 2, (HEX_WIDTH / 2 - hx) * 2, hy * 2 - 1);
      }
      int td = map[i + MAP_TERRAIN_LAYER] * TERRAIN_DATA_SIZE;
      for (int a = 0; a < terrainData[td + TERRAIN_DRAW_AMOUNT_OFFSET]; a++) {
        int terrainColor = terrainData[td + drawRnd.nextInt(2)];
        int xSpread = terrainData[td + TERRAIN_DRAW_X_SPREAD_OFFSET];
      ...
      if (unitImage >= 0) {
        g.fill(borderColor);
        g.rect(x - BITMAP_WIDTH / 2 - UNIT_BORDER_WIDTH, y
          - BITMAP_HEIGHT / 2 - UNIT_BORDER_WIDTH, 
        BITMAP_WIDTH + 2 * UNIT_BORDER_WIDTH, BITMAP_HEIGHT
          + 2 * UNIT_BORDER_WIDTH);
      }
      for (int idr = 0; idr < BITMAP_DATA_HEIGHT; idr++) {
        for (int idc = 0; idc < BITMAP_DATA_WIDTH; idc++) {
          ...
          if (mycolor >= 0) {
            ...
            g.fill(mycolor);
            int x1 = x - BITMAP_WIDTH / 2 + cx * BITMAP_SCALE;
            int y1 = y - BITMAP_HEIGHT / 2 + idr * BITMAP_SCALE;
            g.rect(x1, y1, BITMAP_SCALE, BITMAP_SCALE);
          }
          markImage++;
          unitImage++;
        }
...
void mousePressed() {
  if (mouseY < SCREEN_BUTTON_HEIGHT && mouseX > SCREEN_BUTTON_X) {
    switch (state) {
    case WON_LEVEL:
      level++;
    case FAIL_LEVEL:
  ...
  int i = 0;
  int closestDistSquare = SCREEN_WIDTH * SCREEN_WIDTH + SCREEN_HEIGHT
    * SCREEN_HEIGHT;
  int x = MAP_FIRST_COLUMN_CENTER_X;
  int iClicked = 0;
  int iPrevSelected = -1;
  for (byte c = 0; c < MAP_COLUMNS; c++) {
    ...
    for (byte r = 0; r < MAP_ROWS; r++) {
      ...
      if (distSquare < closestDistSquare && mouseY >= SCREEN_MAP_Y) {
        iClicked = i;
  ...
  byte searchUnit = -1;
  if ((state == SELECT_UNIT_TO_MOVE || state == SHOW_TARGETS)
    ...
    case MARK_ATTACK_TARGET:
      ...
      if (map[iClicked + MAP_ATTACKED_LAYER] > 0 || terrain == CITY
        && searchUnit == ENGINEER) {
        multiplier *= 2;
      }
  ...
  while ( (state & SEARCH_STATE_BIT) > 0) {
    ...
    case FIND_MOVE_TARGETS:
      ...
      nrSearchIterations = unitData[searchUnitIndex
        + UNIT_MOVE_OFFSET];
      map[iClicked + MAP_MOVE_COST_LAYER] = unitData[searchUnitIndex
        + UNIT_MOVE_OFFSET];
    ...
    case FIND_ENEMY_MOVE_TARGETS:
      ...
      nrSearchIterations = unitData[searchUnitIndex
        + UNIT_MOVE_OFFSET];
      map[iPrevSelected + MAP_MOVE_COST_LAYER] = unitData[searchUnitIndex
        + UNIT_MOVE_OFFSET];
      setFoundInLayer = MAP_MARK_LAYER;
      setFoundValue = MARK_MOVE_TARGET;
      resetFoundValue = MARK_NONE;
    ...
    case FIND_ENEMY_ATTACK_TARGETS:
      searchLayer = MAP_RANGE_LAYER;
      range = (byte) (unitData[searchUnitIndex + UNIT_RANGE_OFFSET]);
      nrSearchIterations = range;
      setFoundInLayer = MAP_MARK_LAYER;
    ...
    for (int searchIteration = 0; searchIteration < nrSearchIterations; searchIteration++) {
      for (i = 0; i < MAP_DATA_TOTAL_SIZE; i += MAP_DATA_SIZE) {
        if (state == FIND_FRIENDLY_ZOC) {
          ...
          if (fu > 0) {
            ...
            map[i + MAP_FRIENDLY_THREAT_LAYER] = UNIT_THREAT;
          } 
          else if (eu > 0) {
            map[i + MAP_ENEMY_THREAT_LAYER] = UNIT_THREAT;
            if (eu == ARTILLERY) {
              map[i + MAP_ENEMY_VALUE_LAYER] += ARTILLERY_DEFEND_VALUE;
            }
          }
      ...
      for (int c = 0; c < MAP_COLUMNS; c++) {
        for (int r = 0; r < MAP_ROWS; r++) {
          ...
          if (v > 1) {
            ...
            if (c % 2 == 0) {
              if (c < MAP_COLUMNS - 1) {
                adj[1] = i + MAP_ROWS * MAP_DATA_SIZE;
                if (r < MAP_ROWS - 1) {
                  adj[2] = i + (MAP_ROWS + 1)
                    * MAP_DATA_SIZE;
                }
              ...
              if (c > 0) {
                ...
                adj[5] = i - MAP_ROWS * MAP_DATA_SIZE;
              }
            } 
            else {
              if (c < MAP_COLUMNS - 1) {
            ...
            for (int d = 0; d < 6; d++) {
              if (adj[d] >= 0) {
                ...
                if (searchLayer == MAP_MOVE_COST_LAYER) {
                  int terrain = map[adj[d]
                    + MAP_TERRAIN_LAYER];
                  if (terrain == WATER
                    && map[adj[d]
                    + MAP_FRIENDLY_UNIT_LAYER] == ENGINEER) {
                  } 
                    ...
                    int terrainCost = terrainData[terrain
                      * TERRAIN_DATA_SIZE
                      + TERRAIN_MOVE_COST_OFFSET];
                    ev = (byte) (v - terrainCost);
                  }
                } 
                else if (waterBlocks
                  && map[adj[d] + MAP_TERRAIN_LAYER] == WATER) {
                  ev = 0;
                }
    ...
    for (i = 0; i < MAP_DATA_TOTAL_SIZE; i += MAP_DATA_SIZE) {
      ...
      if ((onlyKeepLayer >= 0 && map[i + onlyKeepLayer] == 0)
        || (notKeepLayer >= 0 && map[i + notKeepLayer] > 0)) {
        map[i + searchLayer] = 0;
      }
    }
    int bestValue = 0;
    for (i = 0; i < MAP_DATA_TOTAL_SIZE; i += MAP_DATA_SIZE) {
      ...
      if (state == SELECT_ENEMY_UNIT_TO_MOVE
        ...
        && map[i + MAP_ENEMY_MOVED_LAYER] == 0) {
        iPrevSelected = i;
        searchUnitIndex = (map[iPrevSelected + MAP_ENEMY_UNIT_LAYER] - 1)
          * UNIT_SIZE;
      } 
      else if (state == FIND_ENEMY_ATTACK_TARGETS
        && map[i + MAP_MARK_LAYER] != MARK_NONE) {
        int value = map[i + MAP_ENEMY_VALUE_LAYER]
