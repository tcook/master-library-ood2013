...
//brick types  0 - empty space  1 - solid  2 - gold 
//creatures  0 - x  1 - y  2 - type (0 = scorpion, 1
//= croc)  3 - Vx
...
public class M extends JFrame {
 ...
 creatures[2] = new int[] { 240, 197, 1, 0 }; // -- DECOMPRESS SPRITES BEGIN
 ...
 for(int s = 0, dataIndex = 0; s < 10; s++) { 	 
	 ...
	 for(int y = 0; y < height; y++, dataIndex++) { 
		 ...
		 for(int j = 0; j < 2; j++, off += 32) { 
			 int row = data & 0xff; data >>= 8; 
		 int Y = (y << 1) + j; 
		 	for(int k = 0; k < 8; k++) { 
		 		int color = (row&1) == 0 ? 0 : (s < 6) ? ((Y < 2) ? 0xFF887000 : (Y < 5) ? 0xFFFC9838 : (Y < 12) ? 0xFF80D010 : 0xFF2A5F00) : (s < 8) ? 0xFFFFFFFF : 0xFF80FF10;
		 		int off2 = off + (k << 1); spriteData[off2] = spriteData[off2 + 1] = spriteData[off2 + 16] = spriteData[off2 + 17] = color; row >>= 1; } } }
	 				for(int i = 0; i < 2; i++) {
	 					int hi = height << 2; 
	 						for(int y = 0; y < hi; y++) { 
	 							for (int left = y << 4, right = left + 15; left < right; left++, right--) { 
	 								int temp = spriteData[left]; spriteData[left] = spriteData[right]; spriteData[right] = temp; } }
 BufferedImage image = new BufferedImage( 16, hi, BufferedImage.TYPE_INT_ARGB_PRE); 
 image.setRGB(0, 0, image.getWidth(), image.getHeight(), spriteData, 0, image.getWidth());
 sprites[s][i] = image; } } // -- DECOMPRESS SPRITES END
		 ...
		 if (bluring > 0) { 
			 if (bluring == 1) { 
				 if (killed) {
					 ...
					 levelState = 2; level++;
					 if (level == 17) { setTitle("Superb!"); } } }
			 				bluring--; blur += blurRate; } 
		 				else { clock++; }
		 					if (level == 17) { continue; } 
		 						if (levelState > 0) { // -- CREATE BOARD BEGIN

 setTitle(new StringBuffer("Pitfall: Scorpion Island -- Stage ") .append(level).toString());
 ...
 snowing = false; } 
 else { snowing = random.nextBoolean();
 ...
 boardMap[y][x] = (y < 13) ? (isNight && y == 0 ? 1 : 0) : snowing ? 3 : 1; } } 
 int goldPile = (random.nextInt(7) << 1) + 1;
 
 for(int i = goldPile; i >= 0; i--) { 
	 for(int j = 12 - (goldPile >> 1) + Math.abs(i - (goldPile >> 1)); 
			 j < 13; j++) { boardMap[j][302+i] = 2; } } 
 		for(int i = 0; i < flakes.length; i++) { flakes[i][0] = random.nextInt(320);
	 ...
	 for(int j = 0; j < 9; j++) {
		 boardMap[4][off+13-j] = boardMap[9][off+j] =
				 boardMap[12-j][off+13] = boardMap[j][off] = 1; }
	 				if (random.nextBoolean()) { // add scorpion 
	 					creature[0] = (off+2) << 4;
	 					creature[1] = 128;
	 					creature[2] = 0;
	 					creature[3] = -1; 
 ...
 if (j % 4 == 0) {
	 boardMap[height1][off + j + (random.nextBoolean() ? 1 : -1)] = boardMap[height1][off + j] = 1;
	 height1 += 5 - random.nextInt(8); 
	 if (height1 < 8) { height1 = 8; } 
	 	else if (height1 > 13) { height1 = 13; } } } } 
	 		else { // pit
	 ...
	 case 6: 
		 case 4:
		 	case 2: //	 stumps/platforms
		 		for(int j = width; j >= 0; j--) { 
		 			for(int k = height2; k < 13; k++) { boardMap[platform ? height2 : k][off + j + width] = 1; }
 }  
		 		// no break by intent 
 case 1: //stump/platform 
	 for(int j = width; j >= 0; j--) {
		 for(int k = height1; k < 13; k++) { 
			 boardMap[platform ? height1 : k][off + j] = snowing ? 3 : 1; } }
	 if (isNight && random.nextBoolean()) {  //add fire chain
		 creature[0] = (off << 4) + 8;
 creature[1] = ((height1+1) << 4);
 creature[2] = 2;
 creature[3] = 6 + random.nextInt(6);
 creature[4] =
 random.nextInt(256); creatureCount++;
 creature = creatures[creatureCount]; } 
	 if (addCreature) {
		 if (random.nextBoolean()) { 
			 ...
			 for(int j = width - 1; j >= 0; j--) {
				 if (random.nextBoolean()) { 
					 ...
					 creature[2] = 1;
					 creatureCount++; } } } } 
	 break; } } // -- CREATE BOARD END

 blur = jumpID = jumpCount = jumping = harryDirection = levelState = flakesCount = 0; harryY = harryX = 32;
 harryVy = harryVx = 0;
 bluring = 50;
 blurRate = 0.02;
 ...
 nextFrameStart =
 System.nanoTime(); }
		 						if (bluring == 0) { // -- MOVE HARRY BEGIN
 
 for(int i = 0; i < 8; i++) {
	 int nextHarryX = ((int)(harryX + harryVx + ((harryVx < 0) ? 6 : 9))) >> 4;
	 boolean hitWall = false;
	 
 if (nextHarryX >= 0 && nextHarryX < 319) {
	 for(int y = (int)harryY - 8; y < (int)harryY + 35; y++) {
		 int Y = y >> 4; if (Y >= 0 && Y < 15 && boardMap[Y][nextHarryX] > 0) {
			 
			 if (boardMap[Y][nextHarryX] == 2) { 
				 foundTreasure = true; } hitWall = true; 
 break; } } 
	 if (!hitWall) { harryX += harryVx; } } hitWall = false;
	 int nextHarryY = ((int)(harryY + harryVy + ((harryVy < 0) ? -8 : 35))) >> 4;
	 boolean touchingIce = false; 
	 for(int x = (int)harryX + 6; x < (int)harryX + 9; x++) {
		 int X = x >> 4;
	 if (nextHarryY >= 0 && nextHarryY < 15 && X >= 0 && X < 319 && boardMap[nextHarryY][X] > 0) {
	 ...
	 if (hitWall) { if (harryVy < 0) { jumping = 1024; harryVy = Math.min(1, -harryVy); }
 else { harryVy = 0; onGround = true; } } 
	 else if (harryVy < 1) { harryVy += 0.0078125; } harryY += harryVy; 
	 if (harryY > 240) { 
		 killed = true; bluring = 50; blur = 1;
 blurRate = -0.02; } 
	 if (foundTreasure) { bluring = 50;
	 blur = 1; 
	 blurRate = -0.02; 
	 running = false;
	 harryVy = 0;
 break; }
	 ...
	 if (K[KeyEvent.VK_LEFT]) {
		 if (onGround) {
			 harryVx = Math.max(-0.25, harryVx - (touchingIce ? 0.0005 : 0.25));
 }
		 else {
		 ...
		 if (onGround) {
			 harryVx = Math.min(0.25, harryVx + (touchingIce ? 0.0005 : 0.25)); }
		 else { harryVx = Math.abs(harryVx); }
		 harryDirection = 0;
		 running = true;
 } 
	 else {
	 ...
	 if (K[KeyEvent.VK_D]) {
		 ...
		 if (jumpID ==
 jumpCount) { jumpCount++; }
		 if (jumping > 0 && onGround) { jumping = 0; } } }
 // -- MOVE HARRY END
 ...
 for(int i = 0; i < creatureCount; i++) { 
	 ...
	 if (Math.abs(creature[0] - cameraX) < 512) {
		 boolean hit = (((int)harryX + 6 >= creature[0] && (int)harryX < creature[0] + 10) || ((int)harryX + 8 >=
				 creature[0] && (int)harryX < creature[0] + 8)) &&
				 ((creature[1] >= (int)harryY && creature[1] < (int)harryY
				 	+ 30) || (creature[1] + 24 >= (int)harryY && creature[1]
				 			< (int)harryY + 6)); 
		 if (creature[2] == 0) {
			 int nextX = (creature[0] + creature[3] + 8) >> 4;
				 	int Y = creature[1] >> 4;
	 if (Y >= 0 && Y < 14 && nextX >= 0 && nextX < 320 && boardMap[Y][nextX] == 0 && boardMap[Y+1][nextX] > 0) {
		 creature[0] += creature[3]; } 
	 else { creature[3] = -creature[3]; } 
	 if (hit) { 
		 killed = true;
		 bluring = 50;
		 blur = 1;
		 blurRate = -0.02; } } 
			 ...
			 for(int j = 0; j < creature[3]; j++) {
				 ...
				 if (fireX >= harryX + 6 && fireX <= harryX + 8 && fireY >= harryY && fireY <= harryY + 30) { 
					 killed = true;
					 bluring = 50; 
					 blur = 1;
					 blurRate = -0.02; } } } } }
 ...
 for(int i = 0; snowing && i < flakes.length && i < flakesCount; i++) { 
	 ...
	 if (i < 768) {
		 ...
		 flake[0] -= 4;
		 flake[1] += 8; }
	 if (flake[0] < 0) { flake[0] += 320; }
	 if (flake[1] >= 240) { flake[1] -= 240; } }
 if (flakesCount < flakes.length && (clock&4) == 4) {
	 flakesCount++; } // -- MOVE SNOW FLAKES END

 cameraX = (int)harryX - 156;
 if (cameraX < 0) { cameraX = 0; } 
 else if (cameraX > 4784) {
	 cameraX = 4784; } // -- UPDATE MODEL END
 
 nextFrameStart += 16666667; }
	 while(nextFrameStart < System.nanoTime());	 //-- RENDER FRAME BEGIN
 ...
 for(int y = 0; y < 15; y++) {
	 for(int x = 0; x < 21; x++) {
		 ...
		 if (element > 0) {
			 ...
			 int Y = (y << 4) + 8;
			 imageGraphics.setColor(element == 3 ? Color.CYAN : element == 1 ? BRICK : (clock&4) == 0 ? Color.YELLOW : Color.WHITE);
		 ...
		 } } imageGraphics.drawImage(sprites[harryVy != 0 ? 5 : running ? ((clock>>2)%5)+1 : 0][harryDirection], (int)harryX - cameraX, (int)harryY, null);
		 for(int i = 0; i < creatureCount; i++) {
			 int[] creature = creatures[i];
			 if (Math.abs(creature[0] - cameraX) < 512) {
				 if (creature[2] == 2) { 
					 ...
					 for(int j = 0; j < creature[3]; j++) {
						 ...
						 imageGraphics.fillOval( (int)(creature[0] - cameraX + 8 * j * cos) - 3, (int)(creature[1] + 8 * j * sin) - 3, 6, 6); } }
				 else { imageGraphics.drawImage( creature[2] == 0 ? sprites[6 + ((clock>>3)&1)][creature[3] < 0 ? 1 : 0] : sprites[8 + ((clock>>7)&1)] [creature[0] - (int)harryX < 0 ? 1 : 0],
 ...
 for(int i = 768; snowing && i < flakes.length && i < flakesCount; i++) {
	 ...
	 if (panelGraphics != null) { 
		 ...
		 panelGraphics.dispose(); }  //-- RENDER FRAME END

 long remaining = nextFrameStart - System.nanoTime();
 if (remaining > 0) {
	 try { Thread.sleep(remaining / 1000000); }
	 catch(Throwable t) { } } } }
