...
//--------------------------------------------------
	 ...
	 * 
	 * Gestures can be recognized at any position, scale, and under any rotation.
	...
	public static class Recognizer {
		...
		public Recognizer( PApplet _parent, Integer _fragmentation, Integer _size, Integer _angle, Integer _angleStep ) {
			...
			while( !stack.empty() ){
				...
				if( result.isEmpty() ){
					...
					if( (distance+localDistance) >= seperator ){
						PVector resample = new PVector(
								pPos.x + ( ( seperator-distance ) / localDistance ) * ( pos.x - pPos.x ),
		...
		protected LinkedList<PVector> getRotateBy( LinkedList<PVector> _points, Float _theta){
			...
			while(iterator.hasNext()){
			    ...
			    result.add( new PVector(
					(point.x-centroid.x)*cos-(point.y-centroid.y)*sin+centroid.x,
					(point.x-centroid.x)*sin+(point.y-centroid.y)*cos+centroid.y,
					0.0f
				));
			}
		...
		protected Rectangle2D.Float getBoundingBox( LinkedList<PVector> _points ){
			...
			return new Rectangle2D.Float( (float)minX, (float)minY, maxX-minX, maxY-minY );
		}

		protected LinkedList<PVector> getScaleToSquare(LinkedList<PVector> _points, Rectangle2D.Float _box ) {
			...
			while(iterator.hasNext()){
				PVector point = (PVector)iterator.next();
				result.add( new PVector(
					   	point.x*( this.size / _box.width ),
					   	point.y*( this.size / _box.height ),
					   	0.0f
				));
			}
		...
		protected synchronized Result check( LinkedList<PVector> _gesture, HashMap<String,Gesture> _templates, HashMap<String, Callback> _binds ) {
			...
			if( this.getPathLength( _gesture ) > this.minLength ){
				...
				for( String key : _binds.keySet() ){
					...
					Float distance = getDistanceAtBestAngle( points, tempTemplate, -(this.angle), this.angle, this.angleStep );
					if ( distance<a ) {
						b = a; a = distance;
		...
		private Float getDistanceAtBestAngle( LinkedList<PVector> _points, Gesture _template, Integer _aDegree, Integer _bDegree, Integer _tresholdDegree ){
...
//--------------------------------------------------
	public static class Gesture {
		...
		protected Gesture( String _name, LinkedList<PVector> _points, Recognizer _recognizer ) {
			...
			this.points = _recognizer.getScaleToSquare( this.points, _recognizer.getBoundingBox( this.points ) );
...
//--------------------------------------------------
	public static class Callback {
		...
		protected Callback( Object _object, String _callback ){  //callback method must be public
			this.object = _object;
			this.callback = _callback;
			try {
			m=this.object.getClass().getMethod(
					this.callback,
					String.class,
					int.class,
					int.class,
