...
/*
Click friendly (blue) unit to select, then click a hex to attack or move to.
Repeat for as many units as you want to move or attack with.
Proceed to the next turn by clicking the button in the upper right corner.
A unit that has already been attacked in the current turn is easier to attack (no risk for your unit, greater chance of eliminating it).
Artillery can attack enemy units up to 3 hexes away. Other units can only attack adjacent hexes.
Enemy units far away are not seen.
Engineers can move into river hexes to let other units cross (the only way to cross rivers).
They are also better than other units at attacking cities.
You win by occupying all cities on the map.
This means you need to leave a unit behind to guard each city you take,
it is not enough to simply pass through them to win, you must have a unit there.
After 15 turns a scenario ends in failure and is restarted.
Complete all 12 scenarios to win the campaign.
 */
public class appWar extends PApplet {
	...
	* HEX_WIDTH;
	...
	private final static int COLOR_OPEN = 0xffffffed;
	...
	public void setup() {
		...
		g = createGraphics(SCREEN_WIDTH, SCREEN_HEIGHT,P2D);
		f = createFont("TimesRoman", 32);
		textFont(f);
	}
	static PFont f;
    static PGraphics g;
	public void draw() {
		...
		for (int c = 0; c < MAP_COLUMNS; c++) {
			...
			+ HEX_HEIGHT / 2;
			for (int r = 0; r < MAP_ROWS; r++, y += HEX_HEIGHT,i += MAP_DATA_SIZE) {
				g.noFill(); g.stroke(0);
				for (int hx = 0; hx <= HEX_WIDTH / 4; hx++) {   //draw hexagons
					int hy = hx * 155 / 100;
					g.rect(x - HEX_WIDTH / 2 + hx,
							y - hy + 2,
							(HEX_WIDTH / 2 - hx) * 2,
							hy * 2 - 1);
				}
				...
				for (int a = 0; a < terrainData[td + TERRAIN_DRAW_AMOUNT_OFFSET]; a++) { //draw terrain features
					int terrainColor =
				...
				if (unitImage >= 0) {
					g.fill(borderColor);
					g.rect(x - BITMAP_WIDTH / 2 - UNIT_BORDER_WIDTH,
	...
	public void mouseClicked() {
		...
		int xx = MAP_FIRST_COLUMN_CENTER_X;
		int iClicked = 0;
		int iPrevSelected = -1;
		for (byte c = 0; c < MAP_COLUMNS; c++) {
			int yy = ((c % 2 == 0) ? MAP_EVEN_ROW_LAYER : 0) + SCREEN_MAP_Y
			+ HEX_HEIGHT / 2;
