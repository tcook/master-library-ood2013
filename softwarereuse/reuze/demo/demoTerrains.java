...
public class demoTerrains extends PApplet {
	...
	{    
	  ...
	  gb_TerrainCamera cam = new gb_TerrainCamera( g_camControl.m_camPos, 
			  new gb_Vector3(0.0f,1.0f,0.0f), 
			  g_camControl.m_camDir,
			  w, h,w);
	  
	  g_materials.SetFovRatio( 45, sc.width);
	  
	  gb_TerrainSunLighting  lgt = new gb_TerrainSunLighting(g_materials, g_sunDir, constrain( g_sunDir.y*4,0,1), g_useClouds);
	...
	public void setup()
	{
	  ...
	  Scenes=new gb_i_TerrainHeightField[] {
			  ...
			  new gb_TerrainEgypt(g_materials),
			  new gb_TerrainRolling(),       
	  ...
	  photographs = new Photo[]{
				new Photo( "Terrain", 0, new gb_Vector3(1,0.5f,1),
						  new gb_Vector3(0.0f,2.0f,0.0f),0,-0.25f, 1789235, false, false),
						new Photo("Valley",0,new gb_Vector3(0.75711614f,0.35974383f,0.54530686f),
						  new gb_Vector3(5.7643337f,0.24380729f,1.2518655f),0.9031241f,-0.013541698f,1789235,true,true), 
						new Photo( "Terrain with Water", 3, new gb_Vector3(1,0.5f,1),
						    new gb_Vector3(0.0f,2.0f,0.0f),0,-0.25f, 1789235, false, false),
						new Photo("Block City",2,new gb_Vector3(0.76745266f,0.13576707f,0.62656504f),
						  new gb_Vector3(-1.9958267f,4.6237698f,-11.27423f),0.915625f,-0.12499994f,1789235,false,false),

						new Photo("Crator",4,new gb_Vector3(0.7794047f,0.420355f,0.46457517f),
						  new gb_Vector3(-1.0333986f,0.100956775f,0.40554827f),0.35312504f,0.099999785f,1789235,true,false),
						new Photo("Reflections",3,new gb_Vector3(-0.25588194f,0.23144478f,0.9385935f),
						  new gb_Vector3(27.797665f,0.15013918f,-2.2791333f),6.0093794f,-0.07499951f,1789235,false,true),
						new Photo("Forests",7,new gb_Vector3(0.71523553f,0.36930996f,0.59333664f),
						  new gb_Vector3(-13.837193f,0.92376465f,-1.1682764f),0.109375134f,-0.2150001f,1789235,false,false),
						new Photo("Snowy Rocks",8,new gb_Vector3(0.70858866f,0.258834f,0.6564352f),
						  new gb_Vector3(-5.7562084f,0.7503891f,5.645005f),0.9f,0.0f,1789235,true,true),
						new Photo("rock",8,new gb_Vector3(-0.1716191f,0.5352259f,-0.82709134f),
						  new gb_Vector3(-6.1476126f,0.16771004f,7.3597717f),2.0187488f,0.1291665f,1789235,false,true),
						new Photo("Crator II",4,new gb_Vector3(0.6666667f,0.33333334f,0.6666667f),
						  new gb_Vector3(11.495641f,5.169949f,8.202846f),-1.6156223f,-0.42916673f,1789235,true,false),
						new Photo("Pyramids",5,new gb_Vector3(0.68041384f,0.27216554f,0.68041384f),
						  new gb_Vector3(6.9793053f,0.736299f,11.575281f),4.284375f,-0.025000066f,1789235,true,false),
						new Photo("sunset",0,new gb_Vector3(0.90130556f,0.05478374f,0.42970574f),
						  new gb_Vector3(-2.2255332f,0.888599f,-2.7276325f),0.89375f,-0.054166615f,167267927,true,true),
						};
	}
	...
	class Photo
	{
	  ...
	  void Output()
	  {
	    ...
	    output.println( "new Vector3("+g_sunDir.x +"," + g_sunDir.y +"," +g_sunDir.z +"),");
	    output.print( "new Vector3("+g_camControl.m_camPos.x +"," + g_camControl.m_camPos.y +"," +g_camControl.m_camPos.z +"),");
	...
	float GetMouseAltitude()
	{
	    ...
	    width, height,width);
	    gb_Ray ray = cam.generateRay( mouseX, height- mouseY);
	    float[] pt_t= gb_TerrainSunLighting.castRay(new gb_TerrainQualitySettings(), Scenes[g_SceneIdx],
	...
	public void mouseDragged()
	{
	  ...
	  gb_Vector3 mpos =  new gb_Vector3( (float)mouseX/(float)width, (float)mouseY/(float)height, 0.0f);
	  if (!ApplyMove)
	  {
	    ApplyMove = true;
...
//	  boolean needNewSmallPhoto = g_CurrentPhotograph!=-1 && g_CurrentPhotograph < photographs.length;
	...
	//  bs/=4;
	//  float eps =0.01f;
	//  Vector3 n = new Vector3 ( bSample(hgts,p.x-eps,p.z,bs) - bSample(hgts,p.x+eps,p.z,bs),
	//  2.0f*eps,
	...
	{
	 return g_NoiseGen.Fbm( x*btile,z*btile, oct,0.5f,2.189f)*bs*0.01f;
	//  return (noise(x*btile,z*btile)*2.-1)*bs;
	}
	float[] getBumpCDs(float x, float z,float bs, float btile, float t)
	{
	  int numBumpOctaves = m_NoiseGradient.CalcNoiseOctaves(g_materials.fovRatio,t, btile*4, 1,1/2.189f);
	...
	void renderImage( gb_TerrainQualitySettings qs, gb_TerrainCamera cam, PImage img, gb_TerrainSunLighting  lgt, gb_i_TerrainHeightField hgts, 
	            ...
	            boolean useGamma)
	{
	  ...
	  for( int i=xStart; i < xEnd; i++ )
	  {
	     ...
	     for( int j=0; j < yres; j++ )
	    {
	      ...
	      {
	        gb_Ray ray = cam.generateRay( (float)i+sampleoffsetx[k], (float)j+sampleoffsety[k]);  
	        float[] pt_t = null;
	        if ( startT != -1.)
	        {
	          pt_t=gb_TerrainSunLighting.castRay( qs, hgts, ray.origin, ray.direction,startT,maxT, 1, g_UseRefinement );
	        }
	  
	        if( pt_t != null && pt_t[1]>0.0f )
	        {
	 //        c = new Vector3(1,1,1);
	          c = lgt.terrainColor( qs,hgts, ray, pt_t[1] ,0, g_UseRefinement);
	        ...
	        else
	        {
	         ...
	         c = lgt.applyClouds(c, ray.origin, ray.direction, pt_t != null && pt_t[1]>0.0f ? pt_t[1] : 50.0f, false, g_doCloudShadows);
	        finalC.add(c);
	      }
	  ...
	  //println("| NumSamples " + g_numHeightSamples + "| Shadow Samples "+g_numHeightShadowSamples);
	}		
}